...
def load_runtime(model_path, tokenizer_path=None, device='cpu', dtype='float32'):
    ...
    if (path / 'tokenizer_config.json').exists():
        ...
        if config.get('tokenizer_class') == 'TokenizersBackend':
            # Transformers 5 export name; same serialized fast-tokenizer backend.
...
def model_binding(model, tokenizer=None):
    from .checkpoints import sha
    # Bind local weights as well as configs; a replaced checkpoint cannot reuse old outputs.
    files={}
    for prefix,directory in [('model',model),('tokenizer',tokenizer or model)]:
        path=Path(directory)
        if not path.is_dir(): raise ValueError('Expected a local model/tokenizer snapshot directory')
        for f in sorted(path.iterdir()):
            if f.is_file() and f.suffix in (('.json','.safetensors','.bin','.model','.jinja') if prefix=='model' else ('.json','.model','.jinja')):
                files[prefix+'/'+f.name]=sha(f)
    return files
