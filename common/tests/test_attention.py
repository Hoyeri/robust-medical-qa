...
from common.attention import receiver_edges, receiver_mask, receivers
from common.representations import Capture, transmission_features
from common.runtime import ModelShape, label_tokens, load_runtime
...
class AttentionTests(unittest.TestCase):
    ...
    def test_baseline_cannot_apply_intervention(self):
        config = read_json(ROOT/'common/configs/attention_blocking.json')
        config['conditions']['baseline']['policy'] = 'all_source_cut'
        payload = {'rows': [{'question_id': 'toy'}]}
        with tempfile.TemporaryDirectory() as d, patch('experiments.attention_blocking.generate') as generate:
            with self.assertRaisesRegex(ValueError, 'Baseline condition must use baseline policy'):
                run(self.model, tiny_tokenizer(), payload, config, read_json(ROOT/'common/prompt.json'), d)
    ...
    def test_local_loader_and_end_to_end(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            run(model,tokenizer,payload,read_json(ROOT/'common/configs/attention_blocking.json'),prompt,root/'attention')
            ...
            analysis=read_json(ROOT/'common/configs/transmission.json')['analysis']
